 ...
 * @param {string} sessionId Session ID to use as database name
 * @returns {Promise<{state: AuthenticationState, saveCreds: () => Promise<void>, closeConnection: () => Promise<void>}>}
 */
export const useMongoDBAuthState = async (uri, sessionId = 'baileys-auth') => {
  try {
    const db = await mongoConnectionManager.getDb(sessionId, uri);
